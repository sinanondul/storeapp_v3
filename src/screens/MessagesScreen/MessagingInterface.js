...
export default class MessagingInterface extends React.Component {
  ...
  componentDidMount() {
        ...
        <TouchableOpacity
          ...
          onPress={() => {
            if (this.props.route.params.chat.groupChatInfo) {
              this.props.navigation.navigate("GroupChatDescription", {
    ...
    this.getParticipants(usersArray).then(() => {
      ...
      const chatRef = firebase
        ...
        .collection("chats")
        .doc(this.props.route.params.chat.id);
        ...
        .onSnapshot((snapshot) => {
          ...
          changes.forEach((change) => {
            if (change.type === "added") {
              ...
              const userItem = this.state.userData.find(
                (user) => user.uid === newMessage.senderId
              );
              let newMessageData;
  ...
  sendMessage(text) {
    ...
    participantIds.forEach((participantId) =>
      this.addToUserMessages(messageItem, chatId, participantId)
    );
  }
