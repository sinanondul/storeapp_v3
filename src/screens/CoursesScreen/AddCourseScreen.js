...
export default class LandingScreen extends React.Component {
  ...
  getCourseItem = (
    ...
    const queryBase = coursesRef
      .where("code", ">=", formattedText.toUpperCase())
      .where("code", "<", limitString);
    ...
    return new Promise((resolve, reject) => {
      coursesQuery.get().then((coursesSnapshot) => {
        coursesSnapshot.forEach((courseDoc) => {
          ...
          if (
            !this.props.courses.some(
              (course) => course.code === courseInfo.code
            ) &&
            !this.state.courses.some(
              (course) => course.code === courseInfo.code
            )
          ) {
  ...
  onChangeSearch = (searchText) => {
    ...
    if (searchText.length > 0) {
      ...
      const limitString = formattedText.replace(
        /.$/,
        nextChar(formattedText.charAt(formattedText.length - 1))
      );
