...
export default class CommentsModal extends React.Component {
    ...
    componentDidMount() {
        let commentsArray = [];
        const postRef = firebase.firestore().collection('posts').doc(this.props.post.id);

        postRef.collection('comments').limit(1).get().then(queryRef => {
            
            if (queryRef.docs.length > 0) {
                this.getCommentBatch({commentsArray: commentsArray, postRef: postRef}).then(lastCommentItem => {
    ...
    getCommentBatch({commentsArray, postRef, batchIndex = 0, batchSize = 10, startAfter = null, orderByUpCount = false}) {
        ...
        return new Promise((resolve, reject) => {
            commentsQuery.get().then(commentsSnapshot => {
                commentsSnapshot.docs.forEach((commentDoc, index, array) => {
                    ...
                    const uped = this.props.userData.upedComments && commentDoc.id in this.props.userData.upedComments;
                    const faved = this.props.userData.favComments && commentDoc.id in this.props.userData.favComments;
